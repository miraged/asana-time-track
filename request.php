...
if ($asana->getResponseCode() == '200' && !empty($userId) ) {
    ...
    if ($workspaceId == '' && $projectId == '') {
        ...
        foreach ($resultJson->data as $workspace) {
            if ($workspace->name == 'Personal Projects')
                continue;
            echo '<button  class=" btn workspace " data-workspace-id="' . $workspace->id . '"><h3>' . $workspace->name . '</h3></button>';
        }
    }
    ...
    if ($workspaceId != '' && $projectId == '') {
        ...
        foreach ($resultJson->data as $project) {
            if ($project->name == 'Personal Projects')
                continue;
            echo '<button  class=" btn project" '
                . 'data-workspace-id="' . $workspaceId .'" '
            . ' data-project-id="' . $project->id . '"><h4>' . $project->name . '</h4></button>';
        }
    }
...
//                continue;
//            } else {
//                 $totals[$taskState['projects']['name']] += $workedHours*60;
//                $totals[$taskState['projects']['name']] += $workedMinutes;
            $totals+= $workedHours*60;
...
//        }
        ...
        $workedTimeTotal = $asana->convertToHoursMins($totals);
        

        // no assigned task is found
        if (!$in)
            echo '<tr><td colspan="5">Sorry, no assigned tasks are found...</td></tr>';

        echo '<tr class="worked_time_line">'
        . '<td>Sum: '
        . $workedTimeTotal
        . '</td><td colspan="2">'
        . '<td class="text_align right">Worked today:</td>'
        . '<td class="worked_time_today">0 hours 0 minutes</td>'
        . '</tr>';
    }
    ...
    echo '<p>ERROR: Something went wrong! Maybe your asana api key does not fit(' . $apiKey . ').<br/> Or you have no internet connection.</p>';
}